...
def run_compare_2025():
    # Load candles
    df = pd.read_csv("artifacts/candles.csv")
    df["open_time"] = pd.to_datetime(df["open_time"])
    
    # Slice candles from 2025-01-01 08:00:00
    df_2025 = df[df["open_time"] >= "2025-01-01 08:00:00"].copy().reset_index(drop=True)
    print("Sliced candles row count:", len(df_2025))
    
    # Run simulation
    out_df = bot.simulate_trades(df_2025, min_ob_quality=1)
    local_trades = out_df.attrs.get("trades_df", pd.DataFrame())
    
    if not local_trades.empty:
        local_trades["entry_time"] = local_trades["entry_idx"].apply(lambda x: df_2025.at[int(x), "open_time"].strftime('%Y-%m-%d %H:%M:%S'))
        local_trades["exit_time"] = local_trades["exit_idx"].apply(lambda x: df_2025.at[int(x), "open_time"].strftime('%Y-%m-%d %H:%M:%S'))
    ...
    sheet_trades = extract_trades_from_csv("scratch/Sheet1.csv")
    sheet_trades["entry_time"] = pd.to_datetime(sheet_trades["entry_time"]).dt.strftime('%Y-%m-%d %H:%M:%S')
    
    print(f"\n==========================================")
    print(f"2025 NO WARMUP COMPARISON (Sheet1)")
    ...
    print(f"Trades only in Local 2025 ({len(only_local)}):")
